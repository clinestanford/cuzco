...
class Pair(models.Model):
    ...
    def checkCointegration(self) -> Tuple[bool, float]:
        data1 = self.retrieveData(self.ticker1.tickerSymbol)
        data2 = self.retrieveData(self.ticker2.tickerSymbol)
    ...
    def retrieveData(self, ticker: str):
        ...
        delta = datetime.timedelta(days=float(self.window))
        oldestDate = today - delta
        data = Price.objects.filter(ticker=ticker, priceData_gte=oldestDate).close
    ...
    def getAveragePrices(self) -> Tuple[float, float]:
        ...
        delta = datetime.timedelta(days=float(self.window))
        oldestDate = today - delta
        tick1Avg = Price.objects.filter(ticker=self.ticker1, priceDate__gte=oldestDate).Aggregate(Avg('close'))["avg__close"]
        tick2Avg = Price.objects.filter(ticker=self.ticker2, priceDate__gte=oldestDate).Aggregate(Avg('close'))["avg__close"]
